...
namespace InfiniteIsland.Engine.Math
{
    ...
    public static class ScaleFuncs
    {
        ...
        {
            return (float)System.Math.Pow(progress, power);
        }

        private static float EaseOutPower(float progress, int power)
        {
            int sign = power % 2 == 0 ? -1 : 1;
            return (float)(sign * (System.Math.Pow(progress - 1, power) + sign));
        }
        ...
        private static float EaseInOutPower(float progress, int power)
        {
            ...
            if (progress < 1)
            {
                return (float)System.Math.Pow(progress, power) / 2f;
            }
            else
            {
                int sign = power % 2 == 0 ? -1 : 1;
                return (float)(sign / 2.0 * (System.Math.Pow(progress - 2, power) + sign * 2));
            }
        }

        private static float SineEaseInImpl(float progress)
        {
            return (float)System.Math.Sin(progress * HalfPi - HalfPi) + 1;
        }

        private static float SineEaseOutImpl(float progress)
        {
            return (float)System.Math.Sin(progress * HalfPi);
        }

        private static float SineEaseInOutImpl(float progress)
        {
            return (float)(System.Math.Sin(progress * Pi - HalfPi) + 1) / 2;
        }
    }
}